...
package body Aw_Config is
	...
	begin
		...
		--TODO: verificar se pode receber wide como parametro
	...
	begin
		Config_Path := Aw_Lib.File_System.To_Vector (
			Ada.Environment_Variables.Value(
				Str_Replace( ' ', '_', 	To_String(Project_Name)	) & "_CONFIG_PATH" 
			)
		);
	exception
	...
	function New_Config_File( N: in String; P: in Parser_Access ) return Config_File is
		...
		begin
			...
			F.File_Name := F.File_Name & To_Unbounded_String(
				Get_File_Name( P.all, N ) );
	...
	procedure Reload_Config( F: in out Config_File ) is
		...
		use Aw_Lib.UWide_String_Ordered_Maps;
	begin
	...
	procedure Set_Section( F: in out Config_File; S: in Wide_String ) is
		-- set the current section of the config file.
	begin
		F.Current_Section := To_Unbounded_Wide_String( S );
	end Set_Section;

	procedure Set_Section( F: in out Config_File; S: in Unbounded_Wide_String) is
		-- set the current section of the config file.
	begin
		F.Current_Section := S;
	end Set_Section;
	...
	function Get_Section( F: in Config_File ) return Wide_String is
		-- return the current section or "" if there is no section active
	begin
		return To_Wide_String( F.Current_Section );
	end Get_Section;

	function Get_Section( F: in Config_File ) return Unbounded_Wide_String is
		-- return the current section or "" if there is no section active
	begin
	...
	function Element( F: Config_File; Key: Unbounded_Wide_String ) return Unbounded_Wide_String is
	...
	function Element( F: Config_File; Key: Wide_String ) return Unbounded_Wide_String is
		-- return the value of element inside the current section with
		-- key Key
		-- if no current section active, return propertie relative
		-- to root section; ie expects Key to be of the form "sectionName.key"
	begin
		return Element( F, To_Unbounded_Wide_String( Key ) );
	end Element;



	function Get_Contents_Map( F: in Config_File ) return Aw_Lib.UWide_String_Ordered_Maps.Map is
